...
- crates.io: `recast-cli`, `recast-mcp` and `recast-core` are all shipped (the bare `recast` name was taken by an unrelated crate). Path-dep version pin in workspace `Cargo.toml` tracks the workspace version; bump in lockstep per release. Publish order matters — `recast-core` first, since the other two pin `=<version>` and the registry must already have it before they resolve.
